...
const DetailInput = ({ listInputs }) => {
  const params = useParams();

  const foundInput = listInputs.find((el) => el.id === parseInt(params.id));
  ...
  const htmlCode = `${foundInput.html}`;
  const scssCode = `${foundInput.scss}`;
    ...
    <>
      ...
      <div className="detail">
        <div className="card detailCard">
          {foundInput ? <>{foundInput.component}</> : "error"}
        </div>
        <div className="html">
